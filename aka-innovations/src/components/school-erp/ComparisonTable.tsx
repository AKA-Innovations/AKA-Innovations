      ...
      <div className="container mx-auto px-4 lg:px-8">
        <div className="max-w-3xl mx-auto text-center mb-16">
          ...
          <h2 className="text-3xl md:text-5xl font-black text-white leading-tight font-dm-sans mb-6">
            How AKA Redefines <span className="text-blue-400">School ERP Software</span>
          </h2>
        ...
        <div className="max-w-5xl mx-auto overflow-x-auto bg-[#0f172a]/60 backdrop-blur rounded-[28px] border border-white/[0.08] p-4 lg:p-8">
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="border-b border-white/[0.08] text-slate-400 text-xs font-bold uppercase tracking-wider">
                ...
                <th className="py-4 px-4 text-white bg-blue-500/10 rounded-t-xl">AKA Agentic ERP</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-white/[0.05] text-xs lg:text-sm">
              {compareCriteria.map((crit, idx) => (
                <tr key={idx} className="hover:bg-white/[0.02] transition-colors duration-200">
                  <td className="py-5 px-4 font-bold text-slate-300">{crit.feature}</td>
                  <td className="py-5 px-4 text-slate-400">{crit.traditional}</td>
                  <td className="py-5 px-4 text-slate-400">{crit.aiErp}</td>
                  <td className="py-5 px-4 text-white bg-blue-500/5 font-semibold border-l border-r border-blue-500/20">{crit.aka}</td>
                </tr>
              ))}
            </tbody>
          </table>
